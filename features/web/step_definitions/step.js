...
When('the Customer is logged into the ghost', async function () {
    ...
    let tittle = await this.driver.$('input[name="identification"]');
    tittle.setValue("h.sosa@uniandes.edu.co");
    await new Promise(resolve => setTimeout(resolve, 2000));
    let pass = await this.driver.$('input[name="password"]');
    pass.setValue("Colombia1*");
    await new Promise(resolve => setTimeout(resolve, 4000));
    await takeScreenshot(this.driver);
    let button = await this.driver.$('button[type="submit"]');
    button.click();
    await new Promise(resolve => setTimeout(resolve, 4000));
});

const resultado = await new Promise((resolve, reject) => {
    resemble(`/Users/betososa/features/ScreenShots/pantalla_${times}-${contador}.png`)
        .compareTo('/ruta/a/imagen_de_referencia.png')
        .onComplete(resolve);
});
...
When('the customer clicks on members and new members', async function () {
    await new Promise(resolve => setTimeout(resolve, 2000));
    let button = await this.driver.$('#ember39');
    button.click();
    await new Promise(resolve => setTimeout(resolve, 4000));
    let button1 = await this.driver.$('button[class="gh-btn gh-btn-green"]');
    button1.click();
    await new Promise(resolve => setTimeout(resolve, 2000));
});
...
When('the customer saves the members', async function () {
    await new Promise(resolve => setTimeout(resolve, 2000));
    let save = await this.driver.$('button[id="ember79"]');
    save.click();
    await new Promise(resolve => setTimeout(resolve, 2000));
});
...
When('the customer enters the new tag', async function () {
    await new Promise(resolve => setTimeout(resolve, 2000));
    let tags = await this.driver.$('a[href="#/tags/"]');
    tags.setValue("Tag Name Test");
    await new Promise(resolve => setTimeout(resolve, 2000));
    let slug = await this.driver.$('a[id="ember99"]');
    slug.click();
    await new Promise(resolve => setTimeout(resolve, 2000));
    let description = await this.driver.$('input[name="name"]');
    description.setValue("name tag");

    await new Promise(resolve => setTimeout(resolve, 2000));
    let des = await this.driver.$('input[name="slug"]');
    des.setValue("description tag");

    await new Promise(resolve => setTimeout(resolve, 2000));
    let desc = await this.driver.$('textarea[name="description"]');
    desc.setValue("description test area");
    await new Promise(resolve => setTimeout(resolve, 4000));
    await takeScreenshot(this.driver);
});
...
Then('the customer saves the tag', async function () {
    await new Promise(resolve => setTimeout(resolve, 2000));
    let save = await this.driver.$('button[class="gh-btn gh-btn-blue gh-btn-icon ember-view"]');
    save.click();
    await new Promise(resolve => setTimeout(resolve, 4000));
    await takeScreenshot(this.driver);
});
...
When('the customer saves the page', async function () {
    await new Promise(resolve => setTimeout(resolve, 2000));
    let button = await this.driver.$('svg[viewBox="0 0 48 48"]');
    button.click();
    await new Promise(resolve => setTimeout(resolve, 2000));
    let button1 = await this.driver.$('button[id="ember276"]');
    button1.click();
});

When('the customer clicks on published and new post', async function () {
    await new Promise(resolve => setTimeout(resolve, 2000));
    let page = await this.driver.$('span[class="flex items-center svg-midgray"]');
    page.click();
    await new Promise(resolve => setTimeout(resolve, 2000));
    let newPage = await this.driver.$('a[id="ember813"]');
    newPage.click();
    await new Promise(resolve => setTimeout(resolve, 2000));
});
...
When('the customer saves the post', async function () {
    ...
    button.click();
    await new Promise(resolve => setTimeout(resolve, 2000));
    let button1 = await this.driver.$('button[id="ember919"]');
    button1.click();
});

Then('the post is register succesful', async function () {
    await new Promise(resolve => setTimeout(resolve, 2000));
    let message = await this.driver.$('a[target="_blank"]');
    message.isDisplayed();
    await new Promise(resolve => setTimeout(resolve, 4000));
    await takeScreenshot(this.driver);
});
...
When('the customer clicks on view site', async function () {
    await new Promise(resolve => setTimeout(resolve, 2000));
    let page = await this.driver.$('a[id="ember928"]');
    page.click();
});

Then('the view site is succesful', async function () {
    await new Promise(resolve => setTimeout(resolve, 3000));
    let cosa = await this.driver.$('a[href="https://ghost-u4ew.onrender.com/"]');
    cosa.isDisplayed();
    await new Promise(resolve => setTimeout(resolve, 4000));
    await takeScreenshot(this.driver);
});
